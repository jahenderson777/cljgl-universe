...
(defn javac []
  ...
  (j/javac "src" {:compile-path     "classes"
                  :compiler-options ["-cp" "src:classes" 
                                     "-target" "11"
                                     "-source" "11" 
                                     "-Xlint:-options"]})
...
(defn start-cider-nrepl
  []
  (.start (Thread. (fn []
                     (println "Starting Cider Nrepl Server")
                     (load-string
                      (str "(ns user
(:require [nrepl.server :as nrepl] [cider.nrepl :as cider]))
(defonce nrepl-server (nrepl/start-server :handler cider/cider-nrepl-handler))
(spit \"./.nrepl-port\" (:port nrepl-server))"))))))

(comment
  (start-cider-nrepl)
  (Thread/sleep 1000)
  (require '[cljgl-universe.start])
  (cljgl-universe.start/-main))
